...
void normalize(Matrix res) {
	double sum;
#	pragma omp parallel for firstprivate(sum) //2
	for (int m = 0; m < res.row; m++) {
		sum = 0;
...
void softmax(Matrix res) {
	double sum;
#	pragma omp parallel for firstprivate(sum) //3
	for (int m = 0; m < res.row; m++) {
		sum = 0;
...
Matrix logits2prediction(Matrix logits) {
	...
	int arg_max;
	double row_max;
#	pragma omp parallel for private(row_max, arg_max) //5
	for (int m = 0; m < logits.row; m++) {
		row_max = -1;
		arg_max = 0;
...
//#	pragma omp parallel for firstprivate(temp_sum, correct) //6
	for (int m = 0; m < row; m++) {
		temp_sum = 0;
		correct = true;
